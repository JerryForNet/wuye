...
namespace HuRongClub.Util
{
    public class Utils
    {
        ...
        public static string Filter(string sInput)
        {
            ...
            string pattern = @"*|and|exec|insert|select|drop|xp_cmdshell|net user|net localgroup|xp_cmdshell|;|#|delete|update|count|master|truncate|declare|char(|mid(|chr(|'";
        ...
        public static string HttpGet(string url)
        {
            ...
            {
                string msg = string.Format("HttpGet请求发送异常，url={0}", url);
            }
            finally
            {
                request = null;
        ...
        public static DateTime Obj2DateTime(object obj)
        {
            ...
            {
                return DateTime.Parse("2220-2-2");
            }
        }
        ...
        /// <returns></returns>
        public static DateTime getWeekUpOfDate(DateTime dt, int day, DayOfWeek weekday, int Number)
        {
            int wd1 = (int)weekday;
            int wd2 = (int)dt.DayOfWeek;
            return wd2 == wd1 ? dt.AddDays(day * Number) : dt.AddDays(day * Number - wd2 + wd1);
        }
        ...
        /// <param name="number"></param>
        /// <returns></returns>
        public static string GetDecimalRemoveZero(string number)
        {
            //为空
            ...
            string[] spliNum = number.Split('.');
            //小数只有一位
            if (spliNum[1].Length == 1)
            {
                //小数为一位，0
                if (spliNum[1] == "0")
                {
            ...
            {
                //小数第二位之后的舍去
                if (spliNum[1].Length >= 3)
                {
                    number = number.Remove(number.LastIndexOf(".") + 3);
                }
                //小数两位都为0
                if (spliNum[1].Substring(0, 1) == "0" && spliNum[1].Substring(1, 1) == "0")
                {
                    return number.Remove(number.LastIndexOf("."));
                }
                //小数第二位为0
                if (spliNum[1].Substring(1) == "0")
                {
                    return number.Remove(number.LastIndexOf(".") + 2);
                }
            }
            return number;
        }
        ...
        /// <returns></returns>
        public static string SupplementZero(string value,int pos)
        {
            if (value.Length < pos)
            {
