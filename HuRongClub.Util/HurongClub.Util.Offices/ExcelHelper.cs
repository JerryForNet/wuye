...
namespace HuRongClub.Util.Offices
{
    /// <summary>
    ///版 本 V1.0
    ///日 期：2015/11/25
    ...
    public class ExcelHelper
    {
        ...
        {
            int colint = 0;
            if (excelConfig.ColumnEntity != null && excelConfig.ColumnEntity.Count > 0)
            {
                List<string> list = new List<string>();
                for (int i = 0; i < excelConfig.ColumnEntity.Count; i++)
                {
                    list.Add(excelConfig.ColumnEntity[i].Column);
                }
                for (int i = dtSource.Columns.Count - 1; i >= 0; i--)
                {
                    DataColumn column = dtSource.Columns[i];

                    int index = list.IndexOf(column.ColumnName); 
                    if (index == -1)
                    {
...

            {
                ...
                si.Author = "HuRongClub"; //填加xls文件作者信息
                si.ApplicationName = "HuRongClub"; //填加xls文件创建程序信息
                si.LastAuthor = "HuRongClub"; //填加xls文件最后保存者信息
                si.Comments = "HuRongClub"; //填加xls文件作者信息
                si.Title = "HuRongClub"; //填加xls文件标题信息
                si.Subject = "HuRongClub";//填加文件主题信息
        ...
        /// <returns></returns>
        public static DataTable ExcelImport(string strFileName, List<int> list = null)
        {
