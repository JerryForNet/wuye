$(function () {
    ...
    $.fn.InitUploader = function (p) {
        var fun = function (parentObj) {
            ...
            if (p.filepath != null) {
                p.sendurl += "&filepath=" + p.filepath;
            }
            if (p.filetypes != null) {
                p.sendurl += "&filetypes=" + p.filetypes;
            }
            if (p.filetype != null) {
                p.sendurl += "&filetype=" + p.filetype;
            }
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (!p.multiple) {
                        ...
                        //parentObj.siblings(".logosrc").attr("src",data.path);
                    } else {
                        if (p.filetype == "0") {
                            addImage(parentObj, data.path, data.size, data.name);
                        } else {
...
//添加图片相册--线下跟进时用
function addImage(targetObj, originalSrc, srcSize, srcname) {
    //插入到相册UL里面
    var newLi = $('<li>'
    + '<input type="hidden" name="hid_photo_path" value="' + originalSrc + '" />'
    + '<input type="hidden" name="hid_photo_size" value="' + srcSize + '" />'
    + '<input type="hidden" name="hid_photo_name" value="' + srcname + '" />'
    + '<span name="span_photo_name">' + CutString(srcname, 15) + '</span>'
    + '<a href="javascript:;" onclick="delImg(this);">删除</a>'
    + '</li>');
    newLi.appendTo(targetObj.siblings(".photo-list").children("ul"));
...
function CutString(inputString, len) {
    ...
    if (inputString.length > len) {
        str = inputString.toString().substring(0, len) + "...";
    } else {
        str = inputString;
    }
    return str;
