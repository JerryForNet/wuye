...
$.fn.printArea = function (opt) {
    ...
    for (var i = 0, len = links.length; i < len; i++) {
        if (links[i].rel === 'stylesheet') {
            //if (opt.usePageStyle || links[i].href.indexOf('hurongclub-report.css') !== -1) {
    ...
    iframe.src = "javascript:void((function(){document.open();document.close()})())";
...
$.fn.FirFoxPrintArea = function (opt) {
    var document = window.document;
    opt = $.extend({
        preview: false,     // 是否预览
        table: false,       // 是否打印table
    ...
    var timer = setInterval(function () {
        if (iframe.contentWindow.canAccess) {
            ...
            var printHtml = $(win.document).context.documentElement.outerHTML;
...
$.fn.printTable = function (opt) {
    ...
    if ($.isbrowsername() == "FF") {
        return this.FirFoxPrintArea(opt);
    } else {
