        ...
        public ActionResult CheckLogin(string username, string password, string verifycode, int autologin)
        {
            ...
            try
            {
                ...
                if (userEntity != null)
                {
                    ...
                    operators.LogTime = DateTime.Now;
                    operators.Token = DESEncrypt.Encrypt(Guid.NewGuid().ToString());
                    operators.OldSystemUserID = userEntity.OldSystemUserID;
                    operators.UserProperty = userEntity.UserProperty;

                    if (userEntity.SecurityLevel != null)
                    {
                        operators.IsLeader = userEntity.SecurityLevel.ToInt();
                    }
                    else
                    {
                        operators.IsLeader = 0;
                    }
                    operators.ManageDepartment = userEntity.ManageDepartment;
                    ...
                    LoginLimit(username, operators.IPAddress, operators.IPAddressName);
                    //写入日志
                    logEntity.OperateAccount = username + "（" + userEntity.RealName + "）";
                    logEntity.ExecuteResult = 1;
