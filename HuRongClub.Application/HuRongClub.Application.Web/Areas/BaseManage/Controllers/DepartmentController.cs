...
namespace HuRongClub.Application.Web.Areas.BaseManage.Controllers
{
    ...
    public class DepartmentController : MvcControllerBase
    {
        ...
        public ActionResult GetTreeListJson(string condition, string keyword)
        {
            ...
            // mapper setting
            AutoMapper.Mapper.Initialize(cfg => { cfg.CreateMap<OrganizeEntity, OrganizeEntity>(); });
            foreach (OrganizeEntity item in organizedata)
            {
                OrganizeEntity items = new OrganizeEntity();
                items = AutoMapper.Mapper.Map<OrganizeEntity>(item);

                TreeGridEntity tree = new TreeGridEntity();
                bool hasChildren = organizedata.Count(t => t.ParentId == items.OrganizeId) == 0 ? false : true;
                if (hasChildren == false)
                {
                    if (items.ParentId != "0")
                    {
                        hasChildren = departmentdata.Count(t => t.OrganizeId == items.OrganizeId) == 0 ? false : true;
                        //if (hasChildren == false)
                        //{
                        //    continue;
                        //}
                ...
                tree.id = items.OrganizeId;
                tree.hasChildren = hasChildren;
                tree.parentId = items.ParentId;
                tree.expanded = true;
                items.EnCode = ""; items.ShortName = ""; items.Nature = ""; items.Manager = ""; items.OuterPhone = ""; items.InnerPhone = ""; items.Description = "";
                items.FullName = "[公司] " + items.FullName;
                string itemJson = items.ToJson();
                itemJson = itemJson.Insert(1, "\"DepartmentId\":\"" + items.OrganizeId + "\",");
            ...
            foreach (DepartmentEntity item in departmentdata)
            {
                ...
                tree.expanded = true;
                tree.hasChildren = hasChildren;
                item.FullName= "[部门] " + item.FullName;
