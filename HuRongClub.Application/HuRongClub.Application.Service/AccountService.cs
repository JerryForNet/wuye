...
using HuRongClub.Application.IService;
...
namespace HuRongClub.Application.Service
{
    /// <summary>
    /// 版 本
    ...
    public class AccountService : RepositoryFactory<AccountEntity>, IAccountService
    {
        ...
        {
            if (this.BaseRepository().IQueryable(t => t.MobileCode == mobileCode).Count() == 0)
            {
                //验证每个IP 不能获取超过5次
                if (this.BaseRepository().IQueryable(t => t.IPAddress == Net.Ip).Count() >= 5)
                {
                ...
                this.BaseRepository().Insert(accountEntity);
        ...
        {
            var data = this.BaseRepository().FindEntity(t => t.MobileCode == accountEntity.MobileCode);
            ...
            accountEntity.EnabledMark = 1;
            this.BaseRepository().Update(accountEntity);
        }
        ...
        public void LoginLimit(string platform, string account, string iPAddress, string iPAddressName)
        {
            ...
            int IsOk = this.BaseRepository().ExecuteByProc("PROC_verify_IPAddress", parameter);
        }
    }
}