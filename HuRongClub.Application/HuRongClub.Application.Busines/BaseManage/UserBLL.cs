...
namespace HuRongClub.Application.Busines.BaseManage
{
    /// <summary>
    /// 版 本
    ...
    public class UserBLL
    {
        ...
        {
            return service.GetListSel(property_id);
        }
        ...
        public UserEntity CheckLogin(string username, string password)
        {
            ...
            if (userEntity != null)
            {
                if (userEntity.EnabledMark == 1)
                {
                    ...
                    bool bl = false;
                    if (dbPassword == userEntity.Password.ToLower())
                    {
                        bl = true;
                    }
                    else
                    {
                        dbPassword = Md5Helper.MD5(DESEncrypt.Encrypt(Md5Helper.MD5(password.ToLower(), 32).ToLower(), userEntity.Secretkey).ToLower(), 32).ToLower();
                        if (dbPassword == userEntity.Password.ToLower())
                        {
                            bl = true;
                        }
                    ...
                    //string dbPassword = Md5Helper.MD5(DESEncrypt.Encrypt(Md5Helper.MD5(password.ToLower(), 32).ToLower(), userEntity.Secretkey).ToLower(), 32).ToLower();
                    if (bl)
                    {
