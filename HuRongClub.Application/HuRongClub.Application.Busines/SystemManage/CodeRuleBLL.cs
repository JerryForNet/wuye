...
namespace HuRongClub.Application.Busines.SystemManage
{
    /// <summary>
    /// 版 本 6.1
    ...
    public class CodeRuleBLL
    {
        ...
        /// <returns>单据号</returns>
        public string GetBillCode(string userId, string moduleId, string enCode = "", IRepository db = null)
        {
            string red = service.GetBillCode(userId, moduleId, enCode);

            if (db != null)
            {
                // 占用单据
                UseRuleSeed(userId, "", enCode, db);
            }
